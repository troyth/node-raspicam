...
RaspiCam.prototype.derivativeOpts = function(){
  ...
  this.opts.filename = this.opts.output.substr( this.opts.output.lastIndexOf("/") + 1 );

  this.opts.filepath = this.opts.output.substr(0, this.opts.output.lastIndexOf("/") + 1 ) || "./";
};
...
RaspiCam.prototype.watchDirectory = function( ) {
  ...
  this.watcher = fs.watch(this.opts.filepath, function(event, filename){
...
RaspiCam.prototype.start = function( ) {
  ...
  switch(this.opts.mode){
    ...
    case 'video':
      ...
      break;
    default:
      console.log('Error: mode must be photo, timelapse or video');
      return false;
  }
