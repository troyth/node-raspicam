...
RaspiCam.prototype.watchDirectory = function() {
  ...
  this.watcher = fs.watch(this.filepath, function(event, filename) {
    ...
    if (filename === self.filename && event === 'rename') {
      self.emit('read', new Date().getTime(), self.filepath, filename);
    } else {
      self.emit(event, new Date().getTime(), self.filepath, filename);
    }
  });
...
RaspiCam.prototype.start = function() {
  ...
  var cmd;
  var cmdOpts = {};
  _.assign(cmdOpts, this.opts);
  switch (this.opts.mode) {
    case 'photo':
      cmd = PHOTO_CMD;
      delete(cmdOpts.timeout);
      delete(cmdOpts.timelapse);
      break;
    case 'timelapse':
      cmd = TIMELAPSE_CMD;

      // if no timelapse frequency provided, return false
      if (typeof cmdOpts.timelapse === 'undefined') {
        this.emit('start', 'Error: must specify timelapse frequency option',
          new Date().getTime());
        return false;
      }
      // if not timeout provided, set to longest possible
      if (typeof cmdOpts.timeout === 'undefined') {
        cmdOpts.timeout = INFINITY_MS;
      }
      break;
    case 'video':
      cmd = VIDEO_CMD;
  ...
  // build the arguments
  var args = [];
  for (var opt in cmdOpts) {
    if (opt !== 'mode') {
      args.push('--' + opt);
      //don't add value for true flags
      if (cmdOpts[opt].toString() !== 'true' &&
          cmdOpts[opt].toString() !== 'false') {
        args.push(cmdOpts[opt].toString());
      }
    }
  }
  ...
  this.childProcess = exec(cmd + ' ' + args.join(' '),
    function(error, stdout, stderr) {
      if (error !== null) {
        this.internalLog('Error when handling execution of child process:  ' +
          error);
      }
    }.bind(this));
