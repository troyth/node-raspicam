...
RaspiCam.prototype.watchDirectory = function(desiredFilename) {
  ...
  this.watcher = fs.watch(this.filepath, function(event, filename) {
    ...
    self.internalLog('raspicam::watcher::event ' + event + 'for filename: ' +
      filename);
    if (event === 'rename' && filename === self.desiredFilename) {
...
RaspiCam.prototype.start = function() {
  ...
  var success = this.watchDirectory(this.opts.output);
  if (success === false) {
