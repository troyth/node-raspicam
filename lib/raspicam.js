...
RaspiCam.prototype.watchDirectory = function() {
  ...
  this.watcher = fs.watch(this.filepath, function(event, filename) {
    ...
    self.internalLog('raspicam::watcher::event ' + event + ' for filename: ' +
      filename);
    console.log('Comparing: ' + self.filepath + ' and ' + filename);
    console.log('self.filepath.indexOf(filename, ' +
      'self.filepath.length - filename.length): ' +
      self.filepath.indexOf(filename,
      self.filepath.length - filename.length));
    var isDesiredFilename = self.filepath.indexOf(filename,
      self.filepath.length - filename.length) !== -1;
    if (isDesiredFilename && event === 'rename') {
