...
RaspiCam.prototype.start = function() {
  ...
  switch (this.opts.mode) {
    ...
    case 'timelapse':
      ...
      if (typeof this.cmdOpts.timelapse === 'undefined') {
        this.emit('start', 'Error: must specify timelapse frequency option',
          new Date().getTime());
        return false;
      }
      // if not timeout provided, set to longest possible
      if (typeof this.cmdOpts.timeout === 'undefined') {
        this.cmdOpts.timeout = INFINITY_MS;
      }
      break;
    case 'video':
      cmd = VIDEO_CMD;
  ...
  // build the arguments
  var args = [];
  for (var opt in this.cmdOpts) {
    if (opt !== 'mode') {
      args.push('--' + opt);
      //don't add value for true flags
      if (this.cmdOpts[opt].toString() !== 'true' &&
          this.cmdOpts[opt].toString() !== 'false') {
        args.push(this.cmdOpts[opt].toString());
      }
    }
  }
